...
function logout() {
  if (state.currentSession) {
    showConfirm("Shift masih aktif! Tutup shift terlebih dahulu.").then(ok => {
      if (ok) tutupShift();
    });
    return;
  }
...
function loadPrintQueue() {
  ...
  if (queue) window.printQueue = JSON.parse(queue);
}
...
async function bukaShift() {
  ...
  try {
    ...
    localStorage.setItem("activeSession", JSON.stringify(state.currentSession));
    showToast(`Shift ${session.shift} dibuka`);
    renderKasir();
  } catch (error) {
    console.error('Error buka shift:', error);
    showToast("Gagal buka shift: " + error.message, 'error');
  }
}
  ...
  const result = await showModal({
    ...
    content: `
      <div class="rekap-card">
        ...
        <div><span>CASH</span><strong> Rp ${formatRupiah(totalCash)}</strong></div>
        <div><span>QRIS</span><strong> Rp ${formatRupiah(totalQRIS)}</strong></div>
        <div><span>Total</span><strong> Rp ${formatRupiah(totalPenjualan)}</strong></div>
      </div>
    `,
    buttons: [
